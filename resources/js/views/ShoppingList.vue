...
onMounted(async () => {
  ...
  } catch { showError('Error al cargar la lista'); }
...
async function remove(id: string) {
  try {
    ...
    if (removed && removed.usage_count > 1) frequent.value.push(removed);
    success('Item quitado');
  } catch { showError('Error al quitar item'); }
}
</script>
